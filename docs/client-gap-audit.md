...
| Finding                                                                                                           | Status here                                                                   | Disposition                                                                                                                                                                                                                                                            |
| ----------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| IncrementalAlterConfigs (append/remove single values without clobbering, Apache Kafka 2.3+)                       | AlterConfigs v0 replace-mode only; sending a resource replaces all its values | **Top open code gap.** Replace-mode can silently drop dynamic defaults an operator set elsewhere. Fix = speak AlterConfigs v1+ flexible with incremental ops + mock/broker tests. Trigger: any user managing broker/topic configs through this client in mixed tooling |
| Partition reassignment, leader election (KIP-460), DescribeLogDirs (franz-go, kafkajs via Confluent REST usually) | Not implemented                                                               | Deferred: cluster-operator workflows; most teams drive these through kafka CLI/Cruise Control anyway                                                                                                                                                                   |
...
| 24-hour/72-hour soak evidence                                       | 30-minute soak passed and recorded; live 24-hour release-profile run in progress at audit time (see HANDOFF wrap-up checklist) | In progress: flip performance-validation status when the run lands                                                                                                                    |
...
0.11, PLAIN/SCRAM/OAUTHBEARER/mTLS auth). The two items to schedule next are
IncrementalAlterConfigs (operator safety) and the KIP-848 adoption decision (ecosystem timing).
