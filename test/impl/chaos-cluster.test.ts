...
const brokers = (
  process.env.CHAOS_BROKERS ?? "127.0.0.1:19092,127.0.0.1:29092,127.0.0.1:39092"
).split(",");
...
function compose(...args: string[]): string {
  const result = Bun.spawnSync(["docker", "compose", "-f", composeFile, "-p", project, ...args], {
    stdout: "pipe",
    stderr: "pipe",
  });
...
const uniqueTopic = (prefix: string) =>
...
async function start(id: number): Promise<void> {
  ...
  await waitFor(async () => {
    ...
    try {
      return (await client.admin().metadata()).brokers.length ? true : undefined;
    } finally {
...
function stop(id: number): void {
  compose("stop", "--timeout", "1", service(id));
}
function pause(id: number): void {
  compose("pause", service(id));
}
function unpause(id: number): void {
  compose("unpause", service(id));
}
...
async function outcome(
  ...
  return Promise.race([
    ...
    Bun.sleep(timeoutMs).then(() => {
      throw new Error(`Operation remained pending for ${timeoutMs}ms`);
    }),
  ]);
}
...
async function leader(client: Kafka, name: string, partition = 0): Promise<number> {
  return waitFor(async () => {
    const found = (await client.admin().metadata([name])).topics
      .find((item) => item.name === name)
      ?.partitions.find((item) => item.id === partition);
...
async function scan(
  ...
  name: string,
  partitions: number,
): Promise<Array<KafkaMessage | ConsumedMessage>> {
  const consumer = client.consumer();
  const assignments = Array.from({ length: partitions }, (_, partition) => ({
    topic: name,
    partition,
  ...
  const highs = await Promise.all(
    assignments.map(({ partition }) =>
      consumer.watermarks(name, partition).then(({ high }) => high),
    ),
  );
...
chaos("three-broker Kafka chaos", () => {
  test("bounds Produce and Fetch when a partition leader is killed, then recovers", async () => {
    ...
    try {
      ...
      await producer.send({
        ...
        timeoutMs: 300,
        acks: "all",
        messages: [{ partition: 0, value: "before-kill" }],
      });
      ...
      await outcome(
        producer.send({
          ...
          timeoutMs: 300,
          acks: "all",
          messages: [{ partition: 0, value: "during-kill" }],
        }),
      );
  ...
  test("times out a blackholed leader and recovers after broker resume", async () => {
    ...
    try {
      ...
      await producer.send({
        topic: name,
        timeoutMs: 300,
        messages: [{ partition: 0, value: "warm" }],
      });
      ...
      expect(
        await outcome(
          producer.send({
            topic: name,
            timeoutMs: 200,
            messages: [{ partition: 0, value: "blackholed" }],
          }),
        ),
      ).toBe("rejected");
      unpause(paused);
      ...
      await waitFor(async () => {
        try {
          await producer.send({
            topic: name,
            timeoutMs: 1_000,
            messages: [{ partition: 0, value: "recovered" }],
          });
          return true;
        } catch {
          return undefined;
      ...
      await waitFor(async () => {
        try {
          return (
            await client
              .producer({ lingerMs: 0 })
              .send({ topic: name, messages: [{ partition: 0, value: "new-leader" }] })
          )[0]?.partition === 0
            ? true
            : undefined;
        } catch {
  ...
  (process.env.CHAOS_NETEM === "1" ? test : test.skip)(
    "stays bounded under delay and packet loss profiles",
    async () => {
      const client = new Kafka(options());
      ...
      try {
        ...
        for (const [delay, jitter, loss] of [
          [10, 2, 0.1],
          [50, 10, 0.5],
          [200, 50, 2],
        ]) {
