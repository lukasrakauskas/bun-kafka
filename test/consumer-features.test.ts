...
describe("Static group membership", () => {
  test("sends group.instance.id through JoinGroup, SyncGroup, Heartbeat, and LeaveGroup", async () => {
      ...
      socket: {
        data(socket, request) {
          ...
          while (offset < request.byteLength) {
            ...
            const memberMetadata = new Writer().i16(0).array(["events"], (writer, topicName) => writer.string(topicName)).bytes(null).result();
            const assignment = new Writer().i16(0).array(["events"], (writer, topicName) => writer.string(topicName).array([0], (item, partition) => item.i32(partition))).bytes(null).result();
            let body: Writer;
    ...
    const kafka = new Kafka({ brokers: [`127.0.0.1:${listener.port}`], retry: { initialBackoffMs: 0, maxBackoffMs: 0 } });
    try {
      const consumer = kafka.consumer({ groupId: "workers", groupInstanceId: "worker-a", heartbeatIntervalMs: 30 });
...
describe("Read-committed isolation", () => {
  test("RecordSetDecoder filters aborted transaction ranges until the commit marker", () => {
    ...
    const batchB = encodeRecordBatch(Array.from({ length: 4 }, (_, i) => ({ value: `drop-${i}` })), Date.now(), "none", { id: 77n, epoch: 0, sequence: 0 }, 4n);
    const combined = new Uint8Array(batchA.byteLength + batchB.byteLength);
    combined.set(batchA);
    combined.set(batchB, batchA.byteLength);
    const decoder = new RecordSetDecoder(combined, "tx", 0, 1, { minOffset: 0n, abortedTransactions: aborted });
    const messages = decoder.read();
    expect(messages.map((message) => decode(message.value))).toEqual(["keep-0", "keep-1", "keep-2", "keep-3"]);
  });
  ...
  test("commit markers resume visibility for later batches of the same producer", () => {
    ...
    const batch1 = encodeRecordBatch([{ value: "a" }, { value: "b" }, { value: "c" }, { value: "d" }], Date.now(), "none", { id: 9n, epoch: 0, sequence: 0 });
    // Control batch carrying a COMMIT marker (value first byte 1) at offset 4.
    const commitMarker = encodeRecordBatch([{ value: new Uint8Array([1]) }], Date.now(), "none", { id: 9n, epoch: 0, sequence: 4, control: true }, 4n);
    // Post-commit data is visible again.
    const batch3 = encodeRecordBatch([{ value: "e" }, { value: "f" }], Date.now(), "none", { id: 9n, epoch: 0, sequence: 5 }, 5n);
    const combined = new Uint8Array(batch1.byteLength + commitMarker.byteLength + batch3.byteLength);
    let at = 0;
    for (const part of [batch1, commitMarker, batch3]) {
      combined.set(part, at);
      at += part.byteLength;
    }
    const messages = new RecordSetDecoder(combined, "tx", 0, 1, { minOffset: 0n, abortedTransactions: aborted }).read();
  ...
  test("other producers stay visible inside aborted ranges", () => {
    ...
    const batch = encodeRecordBatch([{ value: "clean" }], Date.now(), "none", { id: 6n, epoch: 0, sequence: 0 });
    const messages = new RecordSetDecoder(batch, "tx", 0, 1, { minOffset: 0n, abortedTransactions: aborted }).read();
