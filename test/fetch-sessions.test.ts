...
function parseFetchV7(buffer: ArrayBuffer, byteOffset: number, byteLength: number): FetchRequestView {
...
function singleRecordBatch(value: string, baseOffset: bigint): Uint8Array {
  ...
  records.varInt(1 + 1 + 1 + 1 + 1 + valueBytes.byteLength + 1)
    .i8(0).varLong(0).varInt(0)
    .varInt(-1)
    .varInt(valueBytes.byteLength).raw(valueBytes)
    .varInt(0);
  const rawRecords = records.result();
  const w = new Writer(61 + rawRecords.byteLength);
  w.i64(baseOffset).i32(0).i32(-1).i8(2).u32(0)
    .i16(0).i32(0)
    .i64(1700000000000n).i64(1700000000000n)
    .i64(-1).i16(-1).i32(-1).i32(1)
    .raw(rawRecords);
...
function fetchV7Response(sessionId: number, topics: Array<{ name: string; partitions: Array<{ index: number; records: Uint8Array | null }> }>): Writer {
  return new Writer()
    .i32(0) // throttle
    ...
    .array(topics, (writer, t) =>
      writer.string(t.name).array(t.partitions, (partitionWriter, p) =>
        partitionWriter.i32(p.index).i16(0).i64(10).i64(10).i64(0)
          .array([], () => {})
          .bytes(p.records)));
}
...
describe("Fetch sessions (mock broker)", () => {
    ...
    const listener = Bun.listen({
      ...
      socket: {
        data(socket, request) {
          ...
          if (key === 3) {
            respond(socket, correlation, new Writer()
          ...
          if (key === 2) {
            respond(socket, correlation, new Writer().array(["events"], (w, t) => w.string(t).array([0], (p, partition) => p.i32(partition).i16(0).i64(0).i64(10))));
            return;
          }
          if (key === 1) {
            call += 1;
            ...
            if (call === 1) {
              respond(socket, correlation, fetchV7Response(42, [{ name: "events", partitions: [{ index: 0, records: singleRecordBatch("x", 0n) }] }] ));
            } else {
              respond(socket, correlation, fetchV7Response(42, [{ name: "events", partitions: [{ index: 0, records: null }] }]));
            }
            return;
          }
  ...
  test("paused partitions are sent as forgotten topics", async () => {
          ...
          if (key === 3) {
            const metadataBody = new Writer()
              ...
              .array([{ name: "events" }], (writer, item) =>
                writer.i16(0).string(item.name).bool(false).array(
                  [0, 1],
                  (pw, pid) => pw.i16(0).i32(pid).i32(1).array([1], (w) => w.i32(1)).array([1], (w) => w.i32(1)),
                ));
            respond(socket, correlation, metadataBody);
            return;
          }
          if (key === 2) {
            respond(socket, correlation, new Writer().array(["events"], (w, t) => w.string(t).array([0, 1], (p, partition) => p.i32(partition).i16(0).i64(0).i64(10))));
            return;
          }
          if (key === 1) {
            requests.push(parseFetchV7(request.buffer, request.byteOffset, request.byteLength));
            respond(socket, correlation, fetchV7Response(7, [{ name: "events", partitions: [{ index: 0, records: null }, { index: 1, records: null }] }]));
            return;
          }
          respond(socket, correlation, new Writer().i32(0).i16(0));
        },
    ...
    try {
      const consumer = kafka.consumer();
      await consumer.assign([{ topic: "events", partition: 0, offset: 0n }, { topic: "events", partition: 1, offset: 0n }]);
      await consumer.fetch({ maxWaitMs: 20 });
