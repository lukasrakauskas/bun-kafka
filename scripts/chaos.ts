...
const compose = (...args: string[]) => run(["docker", "compose", "-f", composeFile, "-p", project, ...args]);

let exitCode = 1;
try {
  if (await compose("up", "-d", "--wait", "--build")) throw new Error("Could not start the chaos cluster");
