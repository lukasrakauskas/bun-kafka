...
All features listed in packages/docs/src/content/docs/feature-completeness.md are implemented and tested.
...
the wrap-up checklist below, then flip the status lines in packages/docs/src/content/docs/performance-validation.md.
...
- Docs updated: README matrix + packages/docs/src/content/docs/feature-completeness.md.
...
- Long-running soak harness added at scripts/soak.ts (`bun run test:soak` (from packages/kafka)): samples every
  required metric per packages/docs/src/content/docs/performance-validation.md every 10 s, injects bursts, validates a
...
packages/docs/src/content/docs/performance-validation.md.
...
3. Flip status in packages/docs/src/content/docs/performance-validation.md ("Current status" line and the soak
...
`packages/docs/src/content/docs/`. Root scripts delegate with `--cwd`. The live soak writes to
